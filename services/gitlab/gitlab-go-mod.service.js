...
export default class GitlabGoModGoVersion extends GitLabBase {
  ...
  async fetch({ user, repo, branch, filename, gitlab_url }) {
    const project = `${user}/${repo}`
    // https://docs.gitlab.com/ee/api/repository_files.html#get-raw-file-from-repository
    const url = `${gitlab_url}/api/v4/projects/${encodeURIComponent(
      project,
  ...
  async handle(
    { user, repo, branch },
    { filename = 'go.mod', gitlab_url = 'https://gitlab.com' },
  ) {
